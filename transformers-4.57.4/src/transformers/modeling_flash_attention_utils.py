...
def _flash_attention_forward_illava(
    query_states: torch.Tensor,
    key_states: torch.Tensor,
    value_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor],
    ...
    **kwargs,
    ...
    # print(f"cu_seq_lens_q at first: {cu_seq_lens_q}")  # tensor([   0,   64,  128,  192,  256,  320,  384,  448,  512,  576,  640,  704, 768,  832,  896,  960, 1024, 1088, 1152, 1216, 1280, 1344, 1408, 1472, 1536, 1600, 1664, 1728, 1792, 1856, 1920, 1984, 2048, 2112, 2176, 2240, 2304, 2368, 2432, 2496, 2560, 2624, 2688, 2752, 2816, 2880, 2944, 2976], device='cuda:0', dtype=torch.int32)
    # print(f"cu_seq_lens_k at first : {cu_seq_lens_k}")
    # print(f"attention_mask at first : {attention_mask}")  # None
    ...
    if attention_mask is not None:
        ...
        # print(f"cu_seq_lens_q at _upad_input: {cu_seq_lens_q}")
        # print(f"cu_seq_lens_k at _upad_input : {cu_seq_lens_k}")
        # TODO for now this is required to work with
        # https://huggingface.co/kernels-community/metal-flash-sdpa/blob/main/torch-ext/metal_flash_sdpa/__init__.py
        if "mps" in str(q.device):
            cu_seq_lens_k = cu_seq_lens_k.clone()
        ...
        if not return_attn_weights:
            ...
            out_unpad, attn_weights, _ = flash_varlen_fn(
                q,
                k,
                v,
                cu_seqlens_q=cu_seq_lens_q,
    ...
    # Padding free, i.e. sequences flattened into one total sequence
        ...
        # print(f"cu_seq_lens_q at _prepare_from_posids: {cu_seq_lens_q}") # cu_seq_lens_q at _prepare_from_posids: tensor([   0,   64,  128,  192,  256,  320,  384,  448,  512,  576,  640,  704, 768,  832,  896,  960, 1024, 1088, 1152, 1216, 1280, 1344, 1408, 1472, 1536, 1600, 1664, 1728, 1792, 1856, 1920, 1984, 2048, 2112, 2176, 2240, 2304, 2368, 2432, 2496, 2560, 2624, 2688, 2752, 2816, 2880, 2944, 2976],
        # print(f"cu_seq_lens_k at _prepare_from_posids : {cu_seq_lens_k}")
        ...
        if not return_attn_weights:
            ...
            # q :torch.Size([3540, 16, 80])
            # k :torch.Size([3540, 16, 80])
            # v :torch.Size([3540, 16, 80])
            # cu_seq_lens_q : tensor([   0, 3540], device='cuda:0', dtype=torch.int32)
            # cu_seq_lens_k : tensor([   0, 3540], device='cuda:0', dtype=torch.int32)
            # max_length_q : 3540
            # max_length_k : 3540
            out, attn_weights, _ = flash_varlen_fn(
                q,
                k,
                v,
                cu_seqlens_q=cu_seq_lens_q,
    ...
    # No padding
        ...
        if not return_attn_weights:
            ...
            out, attn_weights, _ = flash_fn(query_states, key_states, value_states, return_attn_probs=True, **flash_kwargs)
        if isinstance(out, tuple):
            out = out[0]
