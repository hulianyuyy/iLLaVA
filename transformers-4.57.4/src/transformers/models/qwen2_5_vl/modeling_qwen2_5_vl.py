...
class Qwen2_5_VLVisionBlock(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        print(f"Proceed with forward of Qwen2_5_VLVisionBlock")
        # print(f"cu_seqlens : {cu_seqlens}")
        # print(f"cu_seqlens.shape : {cu_seqlens.shape}")
        # print(f"hidden_states.shape : {hidden_states.shape}")
        # print(f"rotary_pos_emb.shape : {rotary_pos_emb.shape}")  # None 
        # print(f"position_embeddings : {position_embeddings.shape}") # tuple, (emb.cos(), emb.sin())
        # print(f"position_embeddings[0] : {position_embeddings[0].shape}") #torch.Size([5208, 80])
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, illava_config=None, **kwargs) -> torch.Tensor:
        ...
        """
        print("Proceed with forward of Qwen2_5_VisionTransformerPretrainedModel")
        ...
        # print(f"hidden_states.shape first : {hidden_states.shape}")
        ...
        # print(f"hidden_states.shape after selection : {hidden_states.shape}")
        ...
        for layer_num, blk in enumerate(self.blocks):
            ...
            new_image_token_length = hidden_states.shape[0]
            cu_seqlens_now = cu_seqlens_now[cu_seqlens_now < new_image_token_length] 
            cu_seqlens_now = torch.cat([cu_seqlens_now, torch.tensor([new_image_token_length], device=cu_seqlens.get_device(), dtype=cu_seqlens.dtype)], 0)
            ...
            #     **kwargs,
            # )
            # print(f"Processing vision block {layer_num}")
            # print(f"cu_seqlens_now : {cu_seqlens_now}")
            # print(f"hidden_states : {hidden_states.shape}")
            if illava_config is not None and illava_config["enable_illava_vit"] and layer_num in illava_config["illava_vit_k"]:
                hidden_states, position_embeddings = blk(hidden_states, cu_seqlens=cu_seqlens_now, position_embeddings=position_embeddings, perform_self_selection=True, reduce_tokens=illava_config["illava_vit_r"])
            else:
                hidden_states = blk(
                hidden_states,
                cu_seqlens=cu_seqlens_now,
        ...
        # new_image_token_length = hidden_states.shape[0]
        # reverse_indices = reverse_indices[:new_image_token_length]
        # print(f"reverse_indices after selection : {reverse_indices.shape}")
        # print(f"reverse_indices after selection : {reverse_indices}")
        # # hidden_states = hidden_states[reverse_indices, :]
        # hidden_states = hidden_states[reverse_indices, :]

        return hidden_states, cu_seqlens_now
...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        print("Proceed with forward of Qwen2_5_VLAttention")
        ...
        attn_output, attn_weights = attention_interface(
            ...
            position_ids=position_ids[:query_states.shape[0]],  # pass positions for FA2
...
class Qwen2_5_VLTextModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward_illava(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # create position embeddings to be shared across the decoder layers
        position_embeddings = self.rotary_emb(hidden_states, position_ids)
        # print(f"len position_embeddings : {len(position_embeddings)}") # 2
        # print(f"position_embeddings[0].shape : {position_embeddings[0].shape}")  # [3, 1, 820, 128]
        # decoder layers
        all_hidden_states = () if output_hidden_states else None
            ...
            for layer_idx, decoder_layer in enumerate(self.layers):
                ...
                if hidden_states.shape[1] != 1:
                    if layer_idx in illava_llm_k:
                        ...
                        last_layer_attention = layer_outputs[1]  # [nheads, seqlen]
                        ...
                        print(f"last_layer_attention : {last_layer_attention.shape}")
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        second_per_grid_ts (`torch.Tensor` of shape `(num_videos)`, *optional*):
            The time interval (in seconds) for each grid along the temporal dimension in the 3D position IDs.
        """
        print("Undergo forward in Qwen2.5VLModel")
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        if illava_config is not None and illava_config["enable_illava_llm"]:
            print("Proceed with language_model.forward_illava")
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        print("Undergo forward function in Qwen2_5_VLForConditionalGeneration in modeling_qwen2_5_vl.py")
        outputs = self.model(
            input_ids=input_ids,
