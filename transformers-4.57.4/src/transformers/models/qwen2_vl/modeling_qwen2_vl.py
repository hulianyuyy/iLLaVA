    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config._attn_implementation == "flash_attention_2":
            ...
            raise ValueError("Only support Flash_Attention 2 now")
            ...
            **kwargs,
            ...
            if len(cu_seqlens) ==2 : # One image
                hidden_states_1, attn_weights = self.attn(
                    self.norm1(hidden_states), cu_seqlens=cu_seqlens, rotary_pos_emb=rotary_pos_emb, position_embeddings=position_embeddings,return_attn_weights=True, **kwargs,
                )
                ...
                for i in range(len(cu_seqlens)-1):
                    tmp_hidden_states, tmp_attn_weights = self.attn(
                        self.norm1(hidden_states[cu_seqlens[i]:cu_seqlens[i+1]]), cu_seqlens=torch.IntTensor([0, cu_seqlens[i+1] - cu_seqlens[i]]).to(hidden_states.get_device()), rotary_pos_emb=rotary_pos_emb[cu_seqlens[i]:cu_seqlens[i+1]], position_embeddings=position_embeddings[cu_seqlens[i]:cu_seqlens[i+1]], return_attn_weights=True
                    )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            image_embeds, cu_seqlens = self.visual(pixel_values, grid_thw=image_grid_thw, illava_config=illava_config)  # [input_len, channels]
        ...
        if pixel_values_videos is not None:
            ...
            video_embeds, cu_seqlens = self.visual(pixel_values_videos, grid_thw=video_grid_thw, illava_config=illava_config) 
        ...
        if self.illava_config is not None and self.illava_config["enable_illava_llm"]:
            outputs = self.language_model.forward_illava(
            input_ids=None,
            position_ids=position_ids,
            attention_mask=attention_mask,
            past_key_values=past_key_values,
