...
class VisionFlashAttention2(nn.Module):
    ...
    def forward(
        ...
        #     return attn_output
        # else:
        #     # print(f"hidden_states:{hidden_states.shape}")  # [seq_len, channels]
...
#     def forward(self, hidden_states, cu_seqlens, rotary_pos_emb, perform_self_selection=False, track_source=False, reduce_tokens=0) -> torch.Tensor:
...
#         # print(f"hidden_states:{hidden_states.shape}")  # [5280, 1280], [image_token_length, channels]
#         # conduct token merging
#         if perform_self_selection:
#             # merge the minimum values once
#             image_token_length = attn_weights.shape[-1]  # [nheads, seqlen, seqlen] for math attention and [num_images, nheads, seqlen] for flash_attention-2
#             attn_weights_mean = attn_weights.mean(0) # eliminate the heads
#             # the tokens to merge
#             if type(reduce_tokens) is float:
#                 reduce_tokens = math.floor( reduce_tokens * image_token_length)
#                 if reduce_tokens%4 !=0: # for patch merger
#                     if reduce_tokens > 4:
#                         reduce_tokens = (reduce_tokens//4)*4
#                     else:
#                         reduce_tokens = 0
#             indice = attn_weights_mean.mean(0).topk(reduce_tokens+1, largest=False)[1]
#             values = hidden_states[indice]
#             size = torch.arange(reduce_tokens+1, 0, -1)  # from keep_tokens+1 to 1
#             # perform merging
#             hidden_states[indice[-1]] = (size.float().to(values.get_device()) @ values.float() / size.float().sum().to(values.get_device())).half()
#             set_all = set(torch.arange(image_token_length).tolist())
...
#             hidden_states = hidden_states[set_selected]
#             rotary_pos_emb = rotary_pos_emb[set_selected]
#             cu_seqlens[-1] = cu_seqlens[-1] - reduce_tokens
...
class Qwen2VLVisionBlock(nn.Module):
    ...
    def forward(self, hidden_states, cu_seqlens, rotary_pos_emb, perform_self_selection=False, reduce_tokens=0) -> torch.Tensor:
        ...
        if perform_self_selection:
            ...
            new_rotary_pos_emb = []
            new_cu_seqlens = [0]
            # print(f"hidden_states:{hidden_states.shape}") # [3760, 1280]
            # print(f"rotary_pos_emb:{rotary_pos_emb.shape}") # [3760, 40]
            # print(f"cu_seqlens:{cu_seqlens}")  # [0, 3760]
            ...
            # print(f"new hidden_states:{hidden_states.shape}")
            # print(f"new rotary_pos_emb:{rotary_pos_emb.shape}")
            # print(f"new cu_seqlens:{cu_seqlens}")
        hidden_states = hidden_states + self.mlp(self.norm2(hidden_states))
        if perform_self_selection:
            return hidden_states, cu_seqlens, rotary_pos_emb
        else:
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    # Only supports reducing tokens in one layer
    # def forward_illava(
    #     self,
    #     input_ids: torch.LongTensor = None,
    #     attention_mask: Optional[torch.Tensor] = None,
    ...
    #                 # print(f"hidden_states:{hidden_states.shape}")  # [1, 764, 3584]
    #                 # print(f"position_embeddings:{len(position_embeddings)}")  # Tuple of length 2
    #                 # print(f"cache_position:{cache_position.shape}")  # [764]
    #                 # print(f"position_ids:{position_ids.shape}")  # [3, 1, 764]
    #                 # print(f"causal_mask:{causal_mask.shape}")  # None
    #                 # print(f"seq_length_with_past:{seq_length_with_past}") # 764
    ...
    #                 # print(f"last_layer_attention_avg_last_tok:{last_layer_attention_avg_last_tok.shape}") # [764]
    #                 keep_indexs = [ torch.arange(vision_start_position[0]+1, device=device)]
    ...
    #                     values = hidden_states[0, vision_start_position[i]+1+indices[image_token_length-(r+1):]]
    #                     # generate predefined weights
    #                     size = torch.arange(1, r+2)  # from r+1 to 1
    #                     # merge tokens
    #                     hidden_states[0, vision_start_position[i]+1+indices[image_token_length-(r+1)]] = (size.double().to(values.get_device()) @ values.double() / size.double().sum().to(values.get_device())).half()
    #                     set_selected = list(indices[:image_token_length-r] + vision_start_position[i]+1)
    #                     set_selected.sort()
    #                     keep_indexs.append(torch.tensor(set_selected, device=device))
    #                 keep_indexs.append(torch.arange(vision_end_position[-1],seq_length_with_past,device=device))
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                print(f"image_grid_thw:{image_grid_thw}") # [1, 182, 168]
                # print(f"image_grid_thw shape:{image_grid_thw.shape}") # [1, 3] for one image and [2, 3] for two images
                ...
                if n_image_tokens != n_image_features:
                    ...
                    # )
                    # Condcut token merging
                    assert n_image_features < n_image_tokens
                    assert input_ids.shape[0]==1
                    # print(f"vision start position: {torch.where(input_ids[0] == 151652)}")
                    ...
                    # input_ids = torch.cat([input_ids[:, :first_position+n_image_features], input_ids[:, first_position+n_image_tokens:]], 1) #[1, input_len]
                    # inputs_embeds = torch.cat([inputs_embeds[:, :first_position+n_image_features], inputs_embeds[:, first_position+n_image_tokens:]], 1) # [1, input_len, channels]
                    # attention_mask = torch.cat([attention_mask[:, :first_position+n_image_features], attention_mask[:, first_position+n_image_tokens:]], 1)  #[1, input_len]
                    # position_ids = torch.cat([position_ids[:, :, :first_position+n_image_features], position_ids[:, :, first_position+n_image_tokens:]], 2) #[3, 1, input_len]
                image_mask = (
            ...
            if pixel_values_videos is not None:
                # print(f"pixel_values_videos:{pixel_values_videos.shape}")  # [11520, 1176]
                # print(f"video_grid_thw:{video_grid_thw}")  # [4, 40, 72]
                ...
                # print(f"video_embeds:{video_embeds.shape}") # [1476, 3584]
                # print(f"cu_seqlens:{cu_seqlens}")  # [   0, 1476, 2952, 4428, 5904]
