    ...
    def forward(
        ...
        **kwargs,
        ...
        if perform_self_selection:
            ...
            new_pos_emb = [[], []]
            new_cu_seqlens = [0]
            # print(f"hidden_states.shape before merging : {hidden_states.shape}")
            # print(f"cu_seqlens before merging : {cu_seqlens}")
            for i in range(len(attn_weights)):  # The number of images
                # merge the minimum values once
                # print(f"len(attn_weights) : {len(attn_weights)}")  # 1
                # print(f"attn_weights {i} : {attn_weights[i].shape}") # [16, 1404]
                ...
                new_pos_emb[0].append(position_embeddings[0][set_selected])
                new_pos_emb[1].append(position_embeddings[1][set_selected])
            ...
            position_embeddings = (torch.cat(new_pos_emb[0], 0), torch.cat(new_pos_emb[1], 0))
            cu_seqlens = torch.IntTensor(new_cu_seqlens).to(hidden_states.get_device())
            # new_image_token_length = hidden_states.shape[0]
            # cu_seqlens = cu_seqlens[cu_seqlens < new_image_token_length] 
            # cu_seqlens = torch.cat([cu_seqlens, torch.tensor([new_image_token_length], device=cu_seqlens.get_device(), dtype=cu_seqlens.dtype)], 0)
            # print(f"hidden_states.shape after merging : {hidden_states.shape}")
            # print(f"cu_seqlens after merging : {cu_seqlens}")
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, illava_config=None, **kwargs) -> torch.Tensor:
        ...
        for layer_num, blk in enumerate(self.blocks):
            ...
            if illava_config is not None and illava_config["enable_illava_vit"] and layer_num in illava_config["illava_vit_k"]:
                hidden_states, cu_seqlens, position_embeddings = blk(hidden_states, cu_seqlens=cu_seqlens, position_embeddings=position_embeddings, perform_self_selection=True, reduce_tokens=illava_config["illava_vit_r"])
            else:  
                hidden_states = blk(
                    hidden_states,
                    cu_seqlens=cu_seqlens,
            ...
            for layer_idx, decoder_layer in enumerate(self.layers):
                ...
                if hidden_states.shape[1] != 1:
                    if layer_idx in illava_llm_k:
                        ...
                        last_layer_attention = attn_weights  # [nheads, seqlen]
                        # compute average attention over different head
                        # assert last_layer_attention.shape[0] == 1, f"last_layer_attention.shape : {last_layer_attention.shape}"  # Batch size 1
                        # print(f"last_layer_attention : {last_layer_attention.shape}")  # [32, 262]
                        last_layer_attention_avg_last_tok = torch.mean(last_layer_attention, dim=0)
                        ...
                        text_position_ids = text_position_ids[:, keep_indexs] # [1, 170]
                        position_embeddings = (position_embeddings[0][:,  keep_indexs], position_embeddings[1][:, keep_indexs])  # [1, 170, 128], [1, 170, 128]
                        # position_embeddings = self.rotary_emb(hidden_states, position_ids)
