...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward_illava(
        ...
        # image_token_length = illava_llm_image_token_length
            ...
            if use_cache:
                if hidden_states.shape[1] != 1:
                    if layer_idx<illava_llm_k[0]:
                        ...
                        # Perform track source and give visualization
                        if illava_config['illava_track_llm_source'] and source_indice_list != None:
                            import matplotlib.pyplot as plt
                            source_indice_from_image = source_indice_list[0][-1]  # source_indice from 0th video from the last layer performing iLLaVA, with len of num_frames+1, each containing lists of select indice of patches for each frame and the last one in the list is the spatial size (h, w) for raw imaeges. 
                            ...
                            for i in range(num_frames):
                                ...
                                if num_frames>4:
                                    ax[(2*i)//col, (2*i)%col].imshow(raw_frames[0][i])
                                    cmap = plt.cm.get_cmap("RdGy_r")
                                    ax[(2*i+1)//col, (2*i+1)%col].imshow(mask, cmap=cmap)
                                else:
                                    ax[2*i].imshow(raw_frames[0][i])
                                    cmap = plt.cm.get_cmap("RdGy_r")
                                    ax[2*i+1].imshow(mask, cmap=cmap)
                            plt.savefig('attention_map_llm_layer_{}'.format(layer_idx), bbox_inches='tight', dpi=400)
                            plt.close()
